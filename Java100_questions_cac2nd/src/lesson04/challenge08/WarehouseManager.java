...
public class WarehouseManager {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 5; i++) {
			five1 = (int) (Math.random() * 10 % 5) + 1;
			ABKosanArray1[i] = five1;
			for (int j = 0; j < i; j++) {
				if (ABKosanArray1[i] == ABKosanArray1[j]) {
					i--;
					break;
				}
			}
		...
		for (int i = 0; i < 5; i++) {
			five2 = (int) (Math.random() * 10 % 5) + 6;
			ABKosanArray2[i] = five2;
			for (int j = 0; j < i; j++) {
				if (ABKosanArray2[i] == ABKosanArray2[j]) {
					i--;
					break;
				}
			}
		...
		do {
			for (int i = evennumber; i < ABKosanArray2.length; i++) {
				if (ABKosanArray1[i] % 2 == 0) {
					evennumber = i;
					break;
				}
			}
			for (int i = oddnumber; i < ABKosanArray2.length; i++) {
				if (ABKosanArray2[i] % 2 != 0) {
					int num = ABKosanArray1[evennumber];
					ABKosanArray1[evennumber] = ABKosanArray2[i];
					ABKosanArray2[i] = num;
